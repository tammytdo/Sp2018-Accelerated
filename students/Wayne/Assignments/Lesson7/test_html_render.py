...
def test_inouts_OneLineTag():

    e = Title("yolo", "id = 313", "Height = %200 ")
    file_contents = render_result(e)

    print(file_contents)

    assert 'id = "313"' in file_contents
    assert 'Height = "%200"' in file_contents
...
def test_hr():
    ...
    print(file_contents)
    assert file_contents == '<hr width="4000" />'


def test_anchorstrings():
    ...
    assert file_ceontets.endswith('</a>')
