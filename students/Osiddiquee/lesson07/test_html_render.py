...
def test_element_kwargs():
    page = Html()
    page.append('blah blah blah text')
    page.append('blah blah end')
    body_text = Body()
    body_text.append(P("some text content",
                        id="TheList",
                        style="line-height:200%")
                        )
    page.append(body_text)
    file = render_result(page)
    print(file)

    assert 'blah blah blah text' in file
    assert 'blah blah end' in file
    assert '<p id="TheList" style="line-height:200%" >' in file
    assert "</p>" in file
...
def test_selfclosingtag():
    ...
    try:
        selfclosing_text = Br("content to fail")
    except ValueError:
        assert True
    else:
        assert False
